...
def main():
    ...
    file_to_predict = './predic/evidence_Y.csv'
    print(file_to_predict)
    data = pd.read_csv(file_to_predict)
    pre = True
    if pre:
        data = data.reset_index()[['Modified sequence', 'Charge']]
        data = data.rename(columns={'Modified sequence': 'Modified_sequence'})

        data.insert(0, '', range(len(data)))
        print(data.columns)

        print('pre ok!',data['Modified_sequence'] )


    
    outfolder = './predic/prediction'
    ...
    # last_df = pd.DataFrame(seq) 
    # last_df['CCS_prediction'] = preds[:, 0]
    # last_df.to_csv('embeding_c.csv', index=False)
