...
class SampleRT:
    ...
    def fit_tranform(self, data, file_column, peptide_column, RT_column, score_column, target_column):
        ...
        for sample in samples:#不同exp
            ...
            rev_tag_pred=reg_tag.predict(X_rev)
            rev_tag_tag=reg_tag.predict(X_tag)

            pred_rev_tag.loc[dfrev2.index.isin(y_rev.index), sample] =rev_tag_pred# reg_tag.predict(X_rev)
            pred_tag_tag.loc[dftag2.index.isin(y_tag.index), sample] =rev_tag_tag# reg_tag.predict(X_tag)
            # mse_rev_tag = mean_squared_error(y_rev, rev_tag_pred)
            # mse_tag_tag = mean_squared_error(y_tag, rev_tag_tag)
            ...
            rev_tag_pred = reg_rev.predict(X_rev)
            rev_tag_tag = reg_rev.predict(X_tag)
            pred_rev_rev.loc[dfrev2.index.isin(y_rev.index), sample] =rev_tag_pred #reg_rev.predict(X_rev)
            pred_tag_rev.loc[dftag2.index.isin(y_tag.index), sample] =rev_tag_tag #reg_rev.predict(X_tag)
            # mse_rev_rev = mean_squared_error(y_rev, rev_tag_pred)
            # mse_tag_rev = mean_squared_error(y_tag, rev_tag_tag)
        ...
        # self.mse_scores =f"mse_rev_tag: {mse_rev_tag}, mse_tag_tag: {mse_tag_tag},mse_rev_rev: {mse_rev_rev}, mse_tag_rev: {mse_tag_rev}"
...
class MQ_SampleRT:
    ...
    def get_rev(self, df):
        df_rev = df[(df['Leading razor protein'].str.contains('REV__')) & (~df['Leading razor protein'].str.contains('CON__'))]
        df_rev['label'] = 0
        return df_rev
...
def main(evidenve_file, msms_file, lbmsms_file):
    folder_path = '/home/DeepSCP-main/inputData/inhouse/SampleSet'

    #folder_path = '/home/DeepSCP-main/example/data/SampleSet'
    print(' ###################SampleRT###################')
    ...
    SampleRT_scores.to_csv('./outputData/inhouse/SampleRT_scores.csv', index=False)
    plt.figure(figsize=(5, 4))
    # SampleRT_scores = pd.read_csv('../input/SampleRT_scores.csv')
    SampleRT_scores = SampleRT_scores[SampleRT_scores['score'] > 0]

   
    t_stat1, p_value1 = stats.ttest_ind(SampleRT_scores[SampleRT_scores['type'] =='RT(tag|tag)']['score'], SampleRT_scores[SampleRT_scores['type'] =='RT(rev|tag)']['score'])
    t_stat2, p_value2 = stats.ttest_ind(SampleRT_scores[SampleRT_scores['type'] =='RT(tag|rev)']['score'], SampleRT_scores[SampleRT_scores['type'] =='RT(rev|rev)']['score'])


    plt.text(0.5, 0.987, f"p={p_value1:.2e}", ha='center', va='bottom', color='black')
    plt.text(2.5, 0.987, f"p={p_value2:.2e}", ha='center', va='bottom', color='black')


    plt.plot([0, 1], [0.985, 0.985], color='black', lw=1)
    plt.plot([2, 3], [0.985, 0.985], color='black', lw=1)


    plt.plot([0, 0], [0.985, 0.98], color='black', lw=1)  
    plt.plot([1, 1], [0.985, 0.98], color='black', lw=1)  

    plt.plot([2, 2], [0.985, 0.98], color='black', lw=1)  
    plt.plot([3, 3], [0.985, 0.98], color='black', lw=1)  

    colors = ['#FF9999', '#66B3FF', '#99FF99', '#FFCC99']
    sns.boxplot(y='score', x='type', data=SampleRT_scores,palette=colors)
    plt.xticks(fontsize=13, rotation=20)
    plt.yticks(fontsize=13)
    current_ylim = plt.ylim() 


    plt.ylim(current_ylim[0], 1) 
    plt.xlabel('Type', fontsize=15)
    plt.ylabel('$R^2$', fontsize=15)
    plt.tight_layout()
    #plt.savefig('./outputData/inhouse/figure/SampleRT.pdf')
    plt.savefig('./outputData/inhouse/figure/SampleRT.png', format='png', dpi=300)
    ...
    dfRT.to_csv('./outputData/inhouse/dfRT_.csv', index=None)


    print('###################DeepSpec###################')
    #region DeepSpec（msms.txt）
    full_path2 = os.path.join(folder_path, msms_file)
    folder_path2 = '/home/DeepSCP-main/inputData/inhouse/LibrarySet'
    ...
    # region 
    deepspec.ValPlot()
    plt.savefig('./outputData/inhouse/figure/DeepSpec.pdf')
    ...
    dfSP.to_csv('./outputData/inhouse/pre/dfSP_.csv', index=None)
    dfSP.to_csv('./outputData/inhouse/pre/dfSP_.txt', sep='\t', index=None)
    chunk_size = 600000  
    for i in range(0, len(dfSP), chunk_size):
        dfSP.iloc[i:i + chunk_size].to_csv(f'./outputData/inhouse/pre/dfSP_part{i // chunk_size}.csv', index=None)
    ...
    df_pro_ft.to_csv('./outputData/inhouse/DeepSCP_pro_ft.csv')
    df_pep_ft.to_csv('./outputData/inhouse/DeepSCP_pep_ft.csv')
    #endregion
