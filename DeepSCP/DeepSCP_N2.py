...
class DeepSpec:
    ...
    def fit(self, bkmsms):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            with torch.no_grad():
                ...
                #     torch.save(model, 'DeepSpec_nanowell.pkl')
                # else:
                #     counter += 1
                ...
                if val_cosine.mean() >= self.val_cosine_best:
                    ...
                    self.val_loss_best = val_loss
                    self.bestepoch = epoch
                    torch.save(model, 'DeepSpec_N2.pkl')
                else:
                    counter += 1
    ...
    def predict(self, evidence, msms):
        ...
        y_testpreds = []
        model = torch.load('DeepSpec_N2.pkl').to(self.device)
...
class CNN_BiLSTM(nn.Module):
    def __init__(self, batchsize, input_dim, hidden_dim, layer_dim, output_dim, device):
        ...
        self.fc = nn.Sequential(nn.Linear(hidden_size*2 , output_dim),
                                nn.Sigmoid())
        self.fc2 = nn.Sequential(nn.Linear(hidden_size*2 , output_dim),
                                nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        x,_= self.lstm(x)#(46*64)->(46*512)


        x = self.fc(x)#(46*512)->(46*12)
...
class CustomBiLSTM(nn.Module):
    ...
    def forward(self, x):
        device_fw = torch.device('cuda:0')  
        device_bw = torch.device('cuda:3') 
        batch_size, T, input_size = x.shape[0], x.shape[1], x.shape[2]
        h_0_fw = torch.zeros(batch_size, self.hidden_dim).to(device_fw)  
        c_0_fw = torch.zeros(batch_size, self.hidden_dim).to(device_fw)  
        h_0_bw = torch.zeros(batch_size, self.hidden_dim).to(device_bw)
        c_0_bw = torch.zeros(batch_size, self.hidden_dim).to(device_bw)

        out = x
        for layer in range(self.num_layers):
            output_custom =  out.to(device_fw)
            x_reversed = torch.flip(out, [1]).to(device_bw)  

            w_ih_fw = getattr(self.lstm_layer, f'weight_ih_l{layer}').to(device_fw)
            w_hh_fw = getattr(self.lstm_layer, f'weight_hh_l{layer}').to(device_fw)
            b_ih_fw = getattr(self.lstm_layer, f'bias_ih_l{layer}').to(device_fw)
            b_hh_fw = getattr(self.lstm_layer, f'bias_hh_l{layer}').to(device_fw)

            w_ih_bw = getattr(self.lstm_layer, f'weight_ih_l{layer}_reverse').to(device_bw)
            w_hh_bw = getattr(self.lstm_layer, f'weight_hh_l{layer}_reverse').to(device_bw)
            b_ih_bw = getattr(self.lstm_layer, f'bias_ih_l{layer}_reverse').to(device_bw)
            b_hh_bw = getattr(self.lstm_layer, f'bias_hh_l{layer}_reverse').to(device_bw)
            #output_custom_b = out
...
def n2(df_pro):
    ...
    an_cols.to_csv('./example/data/N2/an_cols.csv', index=False)
    ...
    df_pro_ft.to_csv('./example/data/N2/DeepSCP_pro_ft.csv')
    df_pep_ft.to_csv('./example/data/N2/DeepSCP_pep_ft.csv')
    # endregion
...
def main(evidenve_file, msms_file, lbmsms_file):
    folder_path = '/home/DeepSCP-main/inputData/N2/SampleSet'
    print(' ###################SampleRT###################')
    ...
    SampleRT_scores.to_csv('./outputData/N2/SampleRT_scores.csv', index=False)
    plt.figure(figsize=(5, 4))
    ...
    # dfRT(1134335, 119),加了预测RT，score
    dfRT.to_csv('./outputData/N2/dfRT_.csv', index=None)

    # folder_path = '/home/DeepSCP-main/inputData/nanowell/SampleSet'
    full_path2 = os.path.join(folder_path, msms_file)
    folder_path2 = '/home/DeepSCP-main/inputData/N2/LibrarySet'
    ...
    # region 
    deepspec.ValPlot()
    plt.savefig('./outputData/N2/figure/DeepSpec.pdf')
    ...
    del msms, lbmsms
    dfSP.to_csv('./outputData/N2/dfSP_.csv', index=None)
    ...
    (df_pro_ft != 0).sum(0).mean()
    (df_pep_ft.iloc[:, 2:] != 0).sum(0).mean()
    df_pro_ft.to_csv('./outputData/N2/DeepSCP_pro_ft.csv')
    df_pep_ft.to_csv('./outputData/N2/DeepSCP_pep_ft.csv')
    # endregion
